...
import { SearchContainer,SearchContent,Label,FilterButton,DateInput,ContentContainer} from './styles'
...
const SearchComponent = props => {
  ...
  const springProps = useSpring({
    from: startSpring,
    to: props.visible ? startSpring : {  opacity: 1, transform: 'translate3d(0,0,0)' },
    config: { mass: 0.2, tension: 170, friction: 13 }
  })

  return ( <SearchContainer interactive = { props.visible }>
    <animated.div className = {'search__slider' }
    style = {springProps} >
      <SearchContent> <ScrollContainer navbar = {false } color = 'none'>
    <ContentContainer>
    <Container >
    ...
    <Places label = 'Destination' onChange = {location => onSetLocation(location, 'destination')}
    value = {locationReturn.name}/>
    <Label> Departing date </Label>
    <DateInput>
      <DatePicker selected = {dateDeparture } onChange = { date => handleDateChange(date, 'departing') }/>
    </DateInput>

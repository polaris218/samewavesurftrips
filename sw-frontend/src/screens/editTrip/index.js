...
const EditTripScreen = props => {
  ...
  const [state, setState] = useState({
    ...
    date_departure: new Date(props.trips.current.date_times.departure_date_time),
  ...
  const onEditPress = () => {
    ...
    const data = {
        ...
        departing: props.trips.current.departing !== state.departing ? state.departing : JSON.stringify(state.departing),
        departure_date_time: state.date_departure,
        destination: props.trips.current.destination !== state.destination ? state.destination : JSON.stringify(state.destination),
        gender: state.gender,
      ...
      <Container>
          <div className='trip__container'>
            <Card marginBottom={80}>
              ...
              <Places
                label="Destination"
                onChange={((location) => onSetLocation(location, 'destination'))}
              ...
              <DateInput>
                <DatePicker
                  ...
                  onChange={((date) => handleDateChange(date, 'date_departure'))}  
                />
              </DateInput>

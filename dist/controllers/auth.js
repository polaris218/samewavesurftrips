...
function passportLocalStrategy () {
  _passport2.default.use(
    new _passportLocal2.default({ passReqToCallback: true }, function (
      req,
      username,
      password,
      done
      ...
      _user2.default
        ...
        .then(function (user) {
          if (user.length) {
            _bcrypt2.default.compare(password, user[0].password, function (
              err,
              res
            ) {
              if (res) {
...
function refreshToken (req, res, next) {
  ...
  var user = _jsonwebtoken2.default.verify(
    expiredToken,
    _config2.default.auth.secret,
  ...
  if (
    refreshToken in _config2.default.auth.refreshTokens &&
    _config2.default.auth.refreshTokens[refreshToken] == user._id
  ) {
    req.user = user
